    ...
    if len(target_blocks) > 1:
        ...
        for start_block_number_index in range(0, len(target_blocks), block_batch_size):
            ...
            for k in range(end_block_number_index - start_block_number_index):
                ...
                await pricer.get_price_at_block(block)
                # tasks.append(
...
async def getp():
    df = pd.read_csv("~/Downloads/" + 'titan_blocks.csv')
    target_blocks = df['block_number'].unique().tolist()
    target_blocks.sort()
    #target_blocks=target_blocks[:int(len(target_blocks)/50)]
    # target_blocks = [15802092 - i for i in range(1000)] + [15802092 + i for i in range(1000)]
    target_blocks = [15802092 + i for i in range(10000)]
    #print(target_blocks)
    prices = await get_uniswap_historical_prices(target_blocks, "0xaAa5B9e6c589642f98a1cDA99B9D024B8407285A")
    print(prices)
